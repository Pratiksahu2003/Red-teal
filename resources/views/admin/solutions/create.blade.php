...
<div class="max-w-4xl space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-semibold text-brand-900">Create Solution</h1>
            <p class="text-sm text-brand-500 mt-1">Add a new solution to your website.</p>
        </div>
    </div>

    <form method="POST" action="{{ route('admin.solutions.store') }}" enctype="multipart/form-data" class="bg-white rounded-xl border border-brand-200 p-6 space-y-6">
        @csrf
        ...
        @include('admin.components.textarea', ['name' => 'description', 'label' => 'Description', 'value' => old('description'), 'rows' => 6])

        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            @include('admin.components.input', ['name' => 'icon', 'label' => 'Lucide Icon', 'value' => old('icon')])
            @include('admin.components.input', ['name' => 'sort_order', 'label' => 'Sort Order', 'type' => 'number', 'value' => old('sort_order', 0)])
        </div>

        <div x-data="benefitsManager(@js(old('benefits', [''])))" class="space-y-3">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            @include('admin.components.input', ['name' => 'cta_text', 'label' => 'CTA Text', 'value' => old('cta_text')])
            @include('admin.components.input', ['name' => 'cta_url', 'label' => 'CTA URL', 'value' => old('cta_url')])
        </div>
        ...
        <div class="flex justify-end gap-3 pt-4 border-t border-brand-200">
            ...
            <button type="submit" class="inline-flex items-center gap-2 px-5 py-2.5 bg-brand-teal-600 hover:bg-brand-teal-700 text-white text-sm font-medium rounded-lg transition">
                <i data-lucide="save" class="w-4 h-4"></i> Create Solution
            </button>
        </div>
    </form>
</div>
