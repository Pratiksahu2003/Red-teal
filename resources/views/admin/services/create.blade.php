...
<div class="max-w-4xl space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-semibold text-brand-900">Create Service</h1>
            <p class="text-sm text-brand-500 mt-1">Add a new service to your website.</p>
        </div>
    </div>

    <form method="POST" action="{{ route('admin.services.store') }}" enctype="multipart/form-data" class="bg-white rounded-xl border border-brand-200 p-6 space-y-6">
        @csrf
        ...
        <div class="flex justify-end gap-3 pt-4 border-t border-brand-200">
            ...
            <button type="submit" class="inline-flex items-center gap-2 px-5 py-2.5 bg-brand-teal-600 hover:bg-brand-teal-700 text-white text-sm font-medium rounded-lg transition">
                <i data-lucide="save" class="w-4 h-4"></i> Create Service
            </button>
        </div>
    </form>
</div>
