...
<div class="max-w-3xl space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div><h1 class="text-2xl font-semibold text-brand-900">Add Blog Category</h1></div>
    </div>

    <form method="POST" action="{{ route('admin.blog-categories.store') }}" class="bg-white rounded-xl border border-brand-200 p-6 space-y-5">
        @csrf
        @include('admin.components.input', ['name' => 'name', 'label' => 'Name', 'value' => old('name'), 'required' => true])
        @include('admin.components.textarea', ['name' => 'description', 'label' => 'Description', 'value' => old('description'), 'rows' => 3])
        ...
        <div class="flex justify-end gap-3 pt-4 border-t border-brand-200">
            ...
            <button type="submit" class="px-5 py-2.5 bg-brand-teal-600 text-white text-sm rounded-lg">Create Category</button>
        </div>
    </form>
</div>
@endsection
