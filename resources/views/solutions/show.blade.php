...
@section('title', ($solution->meta_title ?? $solution->title) . ' — ' . (settings('company.company_name') ?? 'RedNTeal'))
    ...
    <div class="flex items-start gap-4">
        @if($solution->icon)
            <div class="w-14 h-14 rounded-xl bg-white/10 backdrop-blur flex items-center justify-center shrink-0 border border-white/20">
                <i data-lucide="{{ $solution->icon }}" class="w-7 h-7 text-brand-teal-300"></i>
            </div>
        @endif
        <div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
            <div class="lg:col-span-2 space-y-8">
                ...
                @if($solution->benefits && count($solution->benefits))
                    <div>
                        <h2 class="font-display text-3xl text-brand-900 mb-6">Key Benefits</h2>
